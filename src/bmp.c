...
static int crop(BMPimage *img, int x, int y, int w, int h, BMPimage *resImg) {
...
static int rotate(BMPimage *img, BMPimage *resImg) {
	resImg->width = img->height, resImg->height = img->width;
	if (allocMemoryImg(resImg) != 0) 
		return 1;
	for (int j = 0; j < resImg->height; j++) {
		for (int i = 0; i < resImg->width; i++) {
			resImg->pict[j][i] = img->pict[img->height - 1 - i][j];
		}
	}
	return 0;
}

static int cropAndRotatePicture(BMPimage *img, int x, int y, int w, int h, BMPimage *resImg) {
	BMPimage croppedImg;
	if (crop(img, x, y, w, h, &croppedImg) != 0)
		return 1;
	if (rotate(&croppedImg, resImg) != 0) {
		distructorPicture(&croppedImg);
		return 1;
	}
	distructorPicture(&croppedImg);
...
int cropAndRotate(char *imgFileName, char *resImgFileName, int x, int y, int w, int h) {
	BMPimage img, resImg;
	if (loadBMP(imgFileName, &img) != 0) 
		return 1;
	if (cropAndRotatePicture(&img, x, y, w, h, &resImg) != 0) {
